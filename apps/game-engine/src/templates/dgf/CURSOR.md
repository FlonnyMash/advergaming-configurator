# Cursor instructions — dgf

Template ID: `dgf`  
Scene key: `Dgf` (class `GameScene` in `GameScene.ts`)
...
```
apps/game-engine/src/templates/dgf/
```
...
- `id` must match the folder name `dgf`
...
1. [ ] Copied to `apps/game-engine/src/templates/dgf/`
...
```bash
cd apps/dashboard && pnpm publish-template dgf
```
