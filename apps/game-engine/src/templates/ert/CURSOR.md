# Cursor instructions — ert

Template ID: `ert`  
Scene key: `Ert` (class `GameScene` in `GameScene.ts`)
...
```
apps/game-engine/src/templates/development/ert/
```
...
- `id` must match the folder name `ert`
...
1. [ ] Copied to `apps/game-engine/src/templates/development/ert/`
...
```bash
cd apps/dashboard && pnpm publish-template ert
```
