import JSZip from "jszip";

export interface GenerateTemplateZipParams {
  name: string;
  templateId: string;
}

function buildManifest(name: string, templateId: string): object {
  return {
    meta: {
      templateId,
      name,
      version: "1.0.0",
    },
    branding: {
      primaryColor: { type: "color", value: "#6366f1" },
      backgroundColor: { type: "color", value: "#0f172a" },
    },
    system: {
      gameSpeed: { type: "slider", value: 200, min: 50, max: 800 },
      gravity: { type: "slider", value: 0, min: -1000, max: 1000 },
    },
  };
}

function buildIndexTs(): string {
  return `import manifest from "./manifest.json";
import { GameScene as Scene } from "./GameScene";

export { manifest };
export { Scene };
`;
}

function buildGameSceneTs(sceneKey: string): string {
  return `import Phaser from "phaser";

type ColorControl = { type: "color"; value: string };
type SliderControl = { type: "slider"; value: number; min?: number; max?: number };

type OptionAManifest = {
  meta: { templateId: string; name: string; version: string };
  branding: {
    primaryColor: ColorControl;
    backgroundColor: ColorControl;
  };
  system: {
    gameSpeed: SliderControl;
    gravity: SliderControl;
  };
};

type ConfigUpdatePayload = {
  branding?: OptionAManifest["branding"];
  system?: OptionAManifest["system"];
  manifest?: OptionAManifest;
};

function readColor(control: ColorControl | undefined, fallback: string): string {
  return control?.type === "color" && control.value ? control.value : fallback;
}

function readSlider(control: SliderControl | undefined, fallback: number): number {
  if (control?.type === "slider" && typeof control.value === "number") {
    return control.value;
  }
  return fallback;
}

export class GameScene extends Phaser.Scene {
  static readonly SCENE_KEY = "${sceneKey}";

  private primaryColor = "#6366f1";
  private backgroundColor = "#0f172a";
  private gameSpeed = 200;
  private gravity = 0;
  private accentGraphics?: Phaser.GameObjects.Graphics;

  constructor() {
    super({ key: GameScene.SCENE_KEY });
  }

  create(): void {
    this.drawScene();

    this.game.events.on("CONFIG_UPDATE", (config: ConfigUpdatePayload) => {
      this.applyConfig(config);
    });
  }

  applyConfig(config: ConfigUpdatePayload): void {
    const branding = config.branding ?? config.manifest?.branding;
    const system = config.system ?? config.manifest?.system;

    if (branding) {
      this.primaryColor = readColor(branding.primaryColor, this.primaryColor);
      this.backgroundColor = readColor(
        branding.backgroundColor,
        this.backgroundColor,
      );
    }

    if (system) {
      this.gameSpeed = readSlider(system.gameSpeed, this.gameSpeed);
      this.gravity = readSlider(system.gravity, this.gravity);
    }

    if (this.physics?.world) {
      this.physics.world.gravity.y = this.gravity;
    }

    this.drawScene();
  }

  private drawScene(): void {
    const { width, height } = this.scale;
    this.cameras.main.setBackgroundColor(this.backgroundColor);

    this.accentGraphics?.destroy();
    this.accentGraphics = this.add.graphics();
    this.accentGraphics.fillStyle(
      Phaser.Display.Color.HexStringToColor(this.primaryColor).color,
      1,
    );
    this.accentGraphics.fillCircle(width / 2, height / 2, 48);
  }

  update(_time: number, delta: number): void {
    if (!this.accentGraphics) return;
    const step = this.gameSpeed * (delta / 1000);
    this.accentGraphics.rotation += step * 0.002;
  }
}
`;
}

function toSceneKey(templateId: string): string {
  return templateId
    .split("-")
    .map((part) => part.charAt(0).toUpperCase() + part.slice(1))
    .join("");
}

function buildCursorInstructions(
  name: string,
  templateId: string,
  sceneKey: string,
): string {
  const devPath = `apps/game-engine/src/templates/development/${templateId}`;

  return `# Cursor instructions — ${name}

Template ID: \`${templateId}\`  
Scene key: \`${sceneKey}\` (class \`GameScene\` in \`GameScene.ts\`)

Read this file first. You are extending a **Phaser 3 game template** inside the Advergaming pnpm monorepo. Follow the rules below exactly; do not invent parallel config or bridge protocols.

---

## 1. Install location (required)

Copy this entire folder into the monorepo at:

\`\`\`
${devPath}/
\`\`\`

Expected files:

| File | Role |
|------|------|
| \`manifest.json\` | Template metadata + UI schema (see §3) |
| \`index.ts\` | Registry entry: \`export { manifest, Scene }\` |
| \`GameScene.ts\` | Phaser scene — gameplay only |
| \`CURSOR.md\` | This file (keep updated as you ship features) |

After copying, from the **repository root** run:

\`\`\`bash
pnpm --filter @advergaming/game-engine sync-manifest-registry
\`\`\`

Restart the game engine dev server and open **Studio** (\`/studio\`) — the template should appear in the catalog as **(dev)**.

---

## 2. Monorepo boundaries (do not violate)

| Package / app | Responsibility |
|---------------|----------------|
| \`apps/dashboard\` | Next.js UI, Zustand config, iframe preview, postMessage sender |
| \`apps/game-engine\` | Vite + Phaser canvas, template scenes, DOM overlays, postMessage receiver |
| \`packages/shared\` | \`GameMasterConfig\`, \`BridgeMessage\`, manifest types, config helpers |
| \`packages/studio-engine\` | Studio sidebar + schema-driven controls |

**Never** put dashboard React code in the game engine, or Phaser logic in the dashboard.

---

## 3. Manifest — migrate before shipping

This scaffold uses an **Option A** manifest (\`meta\`, typed \`branding\` / \`system\` controls) for clarity.

The **production catalog** expects a \`TemplateManifest\` JSON Schema manifest (see \`apps/game-engine/src/templates/library/dice-poker/manifest.json\`):

- Top-level: \`id\`, \`version\`, \`author\`, \`previewUrl\`, \`status\`, \`label\`, \`description\`
- \`schema\` with \`properties.branding\` / \`properties.system\` and \`x-control\` on each leaf
- \`id\` must match the folder name \`${templateId}\`

**Your task:** Replace \`manifest.json\` with a valid production manifest before publishing. Use \`gameSchemaFromManifest\` from \`@advergaming/shared\` — controls map to \`config.branding.*\` and \`config.system.*\` paths via \`x-control.targetPath\`.

Until migrated, Studio schema controls may not bind correctly.

---

## 4. Config updates — how the engine applies changes

Flow:

\`\`\`
Dashboard (Zustand) → postMessage UPDATE_CONFIG → game-engine bridge → applyConfig → scene
\`\`\`

Implement **\`TemplateScene\`** in \`GameScene.ts\`:

\`\`\`ts
import type { GameMasterConfig } from "@advergaming/shared";
import type { TemplateScene } from "../../types.ts"; // adjust relative path after install

export class GameScene extends Phaser.Scene implements TemplateScene {
  updateConfig(config: GameMasterConfig): void {
    // Apply branding.theme.*, system.mechanics.*, textures, etc.
  }
}
\`\`\`

Reference: \`apps/game-engine/src/templates/types.ts\`, \`apps/game-engine/src/game/applyMechanics.ts\`, \`apps/game-engine/src/main.ts\` (\`applyConfig\`).

The scaffold also listens for \`this.game.events.on("CONFIG_UPDATE", ...)\` for local testing. **Prefer \`updateConfig\`** — that is what the live bridge calls via \`updatePhaserMechanics\`.

For Base64 player textures, use \`reloadBase64Texture\` from \`apps/game-engine/src/game/reloadBase64Texture.ts\` (always \`textures.remove\` before reload).

---

## 5. DOM vs canvas (strict)

- **Phaser:** physics, sprites, particles, in-game motion only.
- **HTML + Tailwind:** start screen, CTA, lead form, highscores — edited via \`config.branding.domOverlay\` and rendered in \`apps/game-engine/src/overlays/ui-manager.ts\`.

Do not build buttons or forms as Phaser \`GameObjects\`.

Listen for \`window\` event \`GAME_START\` (or \`TemplateScene.start()\`) to begin gameplay after the DOM start screen.

---

## 6. \`index.ts\` registry contract

The engine glob-imports \`./development/*/index.ts\` and expects **named** exports:

\`\`\`ts
import manifest from "./manifest.json";
import { GameScene as Scene } from "./GameScene";

export { manifest };
export { Scene };
\`\`\`

Do not use \`export default\` — \`catalog.ts\` reads \`mod.manifest\` and \`mod.Scene\` directly.

---

## 7. Development checklist

1. [ ] Copied to \`${devPath}/\`
2. [ ] Ran \`sync-manifest-registry\`
3. [ ] Migrated \`manifest.json\` to production JSON Schema + \`x-control\`
4. [ ] \`GameScene\` implements \`TemplateScene\` with \`updateConfig\`
5. [ ] Branding changes apply without iframe reload (test in Studio preview)
6. [ ] DOM overlays configured in manifest / \`domOverlay\` paths if needed
7. [ ] No texture memory leaks on repeated asset upload (use shared reload helper)
8. [ ] Added \`previewUrl\` asset under game-engine \`public/previews/\` if needed

Publish to library when ready:

\`\`\`bash
cd apps/dashboard && pnpm publish-template ${templateId}
\`\`\`

---

## 8. Commands (repo root)

\`\`\`bash
pnpm dev                                    # dashboard + engine (per your workspace scripts)
pnpm --filter @advergaming/game-engine dev  # engine only
pnpm --filter dashboard dev                 # dashboard only
\`\`\`

---

## 9. When editing with Cursor

- Provide **full file paths** and complete TypeScript — no \`// ... rest\` placeholders.
- Match existing templates: \`dice-poker\`, \`clicker\`, \`demo-starter\` under \`apps/game-engine/src/templates/\`.
- If adding manifest controls, update both \`manifest.json\` **and** handle the mapped paths in \`updateConfig\`.
- Point out performance issues (uncleared textures, listeners not removed in \`shutdown()\`).

---

*Generated by Advergaming Studio template scaffold. Update this file as the template evolves.*
`;
}

export async function generateTemplateZip({
  name,
  templateId,
}: GenerateTemplateZipParams): Promise<Blob> {
  const root = templateId;
  const zip = new JSZip();
  const folder = zip.folder(root);

  if (!folder) {
    throw new Error("Failed to create zip folder");
  }

  const displayName = name.trim() || templateId;
  const manifest = buildManifest(displayName, templateId);
  const sceneKey = toSceneKey(templateId) || "Game";

  folder.file("manifest.json", JSON.stringify(manifest, null, 2));
  folder.file("index.ts", buildIndexTs());
  folder.file("GameScene.ts", buildGameSceneTs(sceneKey));
  folder.file("CURSOR.md", buildCursorInstructions(displayName, templateId, sceneKey));

  return zip.generateAsync({ type: "blob" });
}
